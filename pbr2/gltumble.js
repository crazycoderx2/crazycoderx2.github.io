(function (global, factory) {
  ...
  function fromPosition$3(out, pos) {
    out[0] = 1;
    out[1] = 0;
    out[2] = 0;
    out[3] = 0;
  ...
  function fromScale$3(out, scale) {
    out[0] = scale[0];
    out[1] = 0;
    out[2] = 0;
    out[3] = 0;
  ...
  var DEFAULT_CONFIG = Object.freeze({
      homeTilt: 0.4,    // starting tilt in radians
      homeZoom: 0.003,
      homePan: [-0.2, -1.1],
      startSpin: 37.0,   // initial spin in radians per second
  ...
  var Trackball = function Trackball(el, options) {
      ...
      this.previous3Position = this.currentPosition.slice();
      this.currentSpin = 0;
      this.currentTilt = this.config.homeTilt;
      this.currentZoom = this.config.homeZoom;
      this.currentPan = this.config.homePan;
  ...
  Trackball.prototype.handleEvent = function handleEvent (evt) {
      ...
      if (evt.type === 'pointerup' && (evt.buttons == 1)) {
          this.endDrag(pos);
      }
      if (evt.type === 'pointermove' && (evt.buttons == 1)) {
          this.updateDrag(pos);
      }

      if (evt.type === 'pointerdown' && (evt.buttons == 2)) {
          this.startPan(pos);
      }
      if (evt.type === 'pointermove' && (evt.buttons == 2)) {
          this.updatePan(pos);
      }

      if (evt.type === 'wheel') {
          this.updateWheel(evt.deltaY);
      }
  };
  ...
  Trackball.prototype.updateWheel = function updateWheel (deltaY) {
      this.currentZoom *= (deltaY > 0) ? 1.1 : 0.9;
  };
  Trackball.prototype.startPan = function startPan (position) {
      var p = position.slice()
      this.previous3Position = p;
  };
  Trackball.prototype.updatePan = function updatePan (position) {

      var p = position.slice()

      this.currentPan[0] += (p[0] - this.previous3Position[0]) * 0.001;
      this.currentPan[1] -= (p[1] - this.previous3Position[1]) * 0.001;
  ...
  Trackball.prototype.getMatrix = function getMatrix () {
      ...
      var scale = fromScale$3(create$3(), [this.currentZoom, this.currentZoom, this.currentZoom]);
      var transform = fromPosition$3(create$3(), [this.currentPan[0], this.currentPan[1], 0]);

      var result = multiply$3(create$3(), transform, tilt);
      result = multiply$3(result, result, spin);
      return multiply$3(result, result, scale);
  };
